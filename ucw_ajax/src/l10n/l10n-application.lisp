...
(defclass l10n-session-mixin ()
  ((client-timezone :accessor client-timezone-of :initarg :client-timezone :initform +utc-zone+)))
...
(defmethod handle-action :around (action application (session l10n-session-mixin) frame)
  (let ((*default-timezone* (client-timezone-of session)))
    (call-next-method)))
...
(defun render-client-timezone-probe ()
  ...
  (let ((id (js:gen-js-name-string)))
    ...
    (<ucw:script `(dojo.add-on-load (lambda ()
